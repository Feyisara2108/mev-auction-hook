...
/**
 * @notice Deploys MevAuctionHook to Unichain Sepolia using CREATE2 so the hook address
 *         encodes the BEFORE_SWAP permission flag in its lower bits (Uniswap v4 requirement).
 *
 * Usage:
 ...
 *        -vvv
 *
 *   3. Copy the deployed hook address printed to console — you need it for the pool setup.
 */
contract DeployMevAuctionHook is Script {
    ...
    function run() external {
        ...
        bytes memory constructorArgs = abi.encode(
            POOL_MANAGER,
            SMALL_SWAP_THRESHOLD,
            AUCTION_WINDOW_BLOCKS
        );

        // Mine a CREATE2 salt that produces an address whose lower 14 bits encode `flags`.
        // This runs off-chain inside the script (pure computation, no broadcast).
        ...
        console2.log("");
        console2.log("=== MevAuctionHook deployed ===");
        ...
        console2.log("");
        console2.log("Next step: copy address above to HOOK_ADDRESS in .env, then run script/05_CreatePool.s.sol");
    }
}
